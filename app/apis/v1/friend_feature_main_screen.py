  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .toggle-illustration-wrap {
      ...
      height: 72px;
      border-radius: 14px;
      border: 1px solid rgba(188, 157, 127, 0.2);
...
<body>
      ...
      >
        <span class="toggle-illustration-wrap" aria-hidden="true">
          <img
            ...
            alt=""
            src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'%3E%3Crect x='14' y='8' width='36' height='48' rx='7' fill='%23fff6ea' stroke='%23bc8f67' stroke-width='2'/%3E%3Crect x='18' y='14' width='5' height='36' rx='2' fill='%23efcfb1'/%3E%3Cline x1='26' y1='21' x2='44' y2='21' stroke='%23c2865b' stroke-width='2.6'/%3E%3Cline x1='26' y1='29' x2='44' y2='29' stroke='%23c2865b' stroke-width='2.6'/%3E%3Cline x1='26' y1='37' x2='40' y2='37' stroke='%23c2865b' stroke-width='2.6'/%3E%3Ccircle cx='32' cy='49' r='3' fill='%23e29a67'/%3E%3C/svg%3E"
          />
        </span>
      ...
      >
        <span class="toggle-illustration-wrap" aria-hidden="true">
          <img
            class="toggle-illustration"
            alt=""
            src="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'%3E%3Cpath d='M13 52c0-10.2 8.4-18.6 19-18.6S51 41.8 51 52' fill='%23f7e7d4' stroke='%23b98a61' stroke-width='2.2' stroke-linecap='round'/%3E%3Ccircle cx='32' cy='24' r='12' fill='%23f7e7d4' stroke='%23b98a61' stroke-width='2.2'/%3E%3Ccircle cx='27.5' cy='22.5' r='1.3' fill='%23694a32'/%3E%3Ccircle cx='36.5' cy='22.5' r='1.3' fill='%23694a32'/%3E%3Cpath d='M27 28c1.3 1.5 2.9 2.2 5 2.2s3.7-.7 5-2.2' fill='none' stroke='%23694a32' stroke-width='1.8' stroke-linecap='round'/%3E%3C/svg%3E"
          />
        </span>
  ...
  <script>
    ...
    function initFriendToyInteraction() {
      ...
      const secretStorageKey = "friend_toy_secret_opened_date_v2";
      const fortuneDateKey = "friend_toy_daily_fortune_date_v1";
      const fortuneTextKey = "friend_toy_daily_fortune_text_v1";
      const comboWindowMs = 2600;
      const breathingHoldMs = 800;
      const breathingDurationSec = 10;
      let lastToyType = "";
      let lastToyClickedAt = 0;
      let hideSecretTimer = null;
      ...
      function getToyReactionMessages() {
        ...
        return toyMessagesByCharacter[characterId] || toyMessagesCommon;
      }

      const showSecretNote = (text) => {
        secretNote.textContent = text;
        secretNote.classList.add("show");
        if (hideSecretTimer) {
          window.clearTimeout(hideSecretTimer);
        }
        hideSecretTimer = window.setTimeout(() => {
          secretNote.classList.remove("show");
        }, 2600);
      };

      const clearBreathingTicker = () => {
      ...
      const startBreathingSession = () => {
        if (breathingActive) {
          showSecretNote("이미 숨 고르기 진행 중이야 🌿\\n천천히 같이 가보자.");
          return;
        }
        breathingActive = true;
        clearBreathingTicker();

        let remain = breathingDurationSec;
        bubble.textContent = "좋아, 지금부터 10초만\\n천천히 숨 고르자 🌿";
        showSecretNote(`숨 고르기 시작 🌿\\n${remain}초`);

        breathingIntervalId = window.setInterval(() => {
          remain -= 1;
          if (remain > 0) {
            const guide = (remain % 2 === 0) ? "천천히 들이마시기" : "길게 내쉬기";
            showSecretNote(`숨 고르기 ${remain}초\\n${guide}`);
            return;
          }
          clearBreathingTicker();
          breathingActive = false;
          bubble.textContent = "좋았어, 끝까지 잘했어 🤍\\n지금처럼 천천히 가보자.";
          showSecretNote("10초 완료 ✨\\n정말 잘했어, 마음이 한결 편해졌길 바라!");
        }, 1000);
      };

      const onToyClick = (target, toyType) => {
        ...
        const lastOpenedDate = window.localStorage.getItem(secretStorageKey) || "";
        const isCombo =
          lastToyType &&
          lastToyType !== toyType &&
          (now - lastToyClickedAt) <= comboWindowMs;

        lastToyType = toyType;
        lastToyClickedAt = now;

        // 2) 오늘의 한 줄 운세: 장난감 상호작용 하루 1회
        const fortuneShownDate = window.localStorage.getItem(fortuneDateKey) || "";
        if (fortuneShownDate !== today) {
          const fortune = pickRandom(dailyFortunes) || dailyFortunes[0];
          window.localStorage.setItem(fortuneDateKey, today);
          window.localStorage.setItem(fortuneTextKey, fortune);
          showSecretNote(`오늘의 한 줄 🍀\\n${fortune}`);
        }

        // 몰래 이벤트: 하루 1회만 노출
        if (isCombo) {
          if (lastOpenedDate !== today) {
            window.localStorage.setItem(secretStorageKey, today);
            showSecretNote(pickRandom(secretMessages) || secretMessages[0]);
          } else {
            showSecretNote("오늘 비밀메모는 이미 확인했어 🌙\\n내일 다시 열어보자!");
          }
        }
      };

      const bindToy = (el, toyType) => {
        const run = () => onToyClick(el, toyType);
        let holdTimer = null;
